  ...
  const handleForm = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      for (let i = 0; i < questions.length; i++) {
        ...
        const questionId = (questions[i] as any)._id;
      ...
      <div className="flex flex-col">
              ...
              <div key={question._id}>
                ...
                {question.options && question.options.length && question.options.length >= 1
                  ? (options[question.title] as any).options.map((option: any) => {
                      return <InputCustom key={option._id} questionTitle={question.title} optionId={option._id} optionName={option.name} />;
                    })
                  : null}
              </div>
            ))
        ...
        <div className="max-w-2xl mx-auto">
          <label htmlFor="message" className="block mb-2 text-sm font-medium text-gray-900 dark:text-gray-400">
            This textarea bar component is part of a larger, open-source library of Tailwind CSS components. Learn more by going to the official{" "}
          </label>
        ...
        <div className="flex justify-center">
          <button className="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded">Enviar Encuesta</button>
        </div>
      </div>
    </form>
  );
